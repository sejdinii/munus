...
### Batch 4 — 2026-07-25 (in progress — geography gaps + Workable/SmartRecruiters rebalance)
...
at 6 and 8 of 116). This first tranche below focuses on those five
countries; a second tranche (below it, appended later this same run) adds
more general Ashby/Lever/SmartRecruiters accounts to reach 30+ total.
...
(Batch 4 continues below with a second tranche to reach 30+ — see further
down this file, appended after additional research this same run.)
...
- **NEW this run — Lever also has an EU-hosted variant, exactly parallel to Greenhouse's `.eu.` host issue (founder, Batch 4, 2026-07-25):** Silverfin (Belgium) resolves its candidate board at `jobs.eu.lever.co/silverfin`, not the default `jobs.lever.co`. Confirmed via Lever's own public postings-api documentation (github.com/lever/postings-api): "Some accounts are hosted in the EU region and answer on `https://api.eu.lever.co/v0/postings/{site_slug}` instead. EU accounts use `api.eu.lever.co` and `jobs.eu.lever.co`." This has been undocumented risk in this seed list for four batches — every Lever row accumulated so far (19 companies across Batches 1–4) was recorded against the default `api.lever.co` host and has NOT been individually checked for whether it's actually EU-hosted. Unlike the Greenhouse EU-host situation (where the API-host equivalence was merely "undocumented, needs testing"), this is now a **confirmed, documented dual-host requirement** straight from Lever's own docs — the ingestion adapter's dual-host fallback logic (already flagged as required for Greenhouse) must also cover Lever, and this should be treated as slightly higher-confidence/higher-priority than the Greenhouse case specifically because Lever's own documentation confirms the split rather than it being inferred from candidate-facing URLs alone.
