# BACKLOG.md - founder's research output, orchestrator's inbox
# The founder appends here continuously. The orchestrator reads it ONLY at
# checkpoints and marks items ACCEPTED (goes into FEATURES.md) or REJECTED
# (stays here with a reason, so it is never re-proposed blind).

## URGENT
(founder: only genuine wave-invalidating discoveries; otherwise leave empty)

## PROPOSED FEATURES
| Feature | Evidence (source) | Impact (blocking/valuable/later) | Touches | Verdict |
|---|---|---|---|---|
| | | | | |

## DESIGN INTEL
- (pattern worth cloning - app, source, date)
- 2026-08-08 · Visual-mass research (Mobbin pull, 40+ screens): nobody current
  ships "mostly white + scattered chips" — category analogs Angi (flow
  413f781a) / Airtasker (flow 93cebe6f) run 35-40% color mass in a top band
  with organic blobs + overlapping content sheet (IMPLEMENTED bw0). Duolingo
  (e2891a87): 3-layer selected rows at 2px (IMPLEMENTED). Redfin/Deezer:
  gradient CTAs (IMPLEMENTED). Superlist (e4944ab3): rotated MINI PRODUCT
  CARDS fanned as welcome hero — FOR DECK WAVE: replace welcome orbit cards
  with real mini job-card thumbnails once the swipe card component exists
  (previews the actual product). Airtasker sticker-squiggle accents: cheap
  craft signal, optional. Avoid: Bumble binary-question layout (50% dead
  below options), soft-drop-shadow-as-primary-depth (reads 2019).
- 2026-08-08 · impeccable skill vendored at .claude/skills/impeccable (+ its
  finish/asset/documenter subagents in .claude/agents; user-requested, Apache
  2.0). Applied its polish playbook + craft floor to bw0: SVG icon system
  replacing unicode glyphs, themed browser surfaces (selection/caret/
  placeholder/scrollbar/underline), file-card promoted to system CSS,
  progress bar animates transform not width (its detector's finding).
  Craft-floor items OVERRIDDEN by our committed world, per its own "brief
  wins" rule: overline/eyebrow pattern (prototype-pinned), hard ink-offset
  shadows (prototype's neo-brutalist accents), system font stack (iOS-native
  PWA decision). Its detector (scripts/detect.mjs) is now part of the
  pre-commit bar for UI work. Follow-up on offer: `/impeccable init` to
  capture PRODUCT.md; DESIGN.md could be generated from CONTRACTS.md.
- 2026-08-08 · ui-ux-pro-max skill vendored at .claude/skills/ui-ux-pro-max
  (user-requested, MIT). Useful: touch-first effect specs (press scale 0.97 —
  implemented), pre-delivery checklist, searchable UX guidelines. Its
  design-system generator proposed orange/Fredoka/marketplace-landing —
  REJECTED: Scout's pink identity is a recorded decision; wholesale template
  swaps are the generic-design trap. Query it per-topic, never adopt whole.
- 2026-08-08 · Sign-in consensus (researched): unified "Continue" screen, Google
  first then Apple (PWA — not bound by Apple's native 4.8 prominence rule),
  full-width stacked, Google white+border+color G, Apple solid black on light bg,
  legal copy ABOVE buttons (Indeed/Glassdoor placement), passwordless. Sources:
  Mobbin — Indeed ac3a27e4, Glassdoor 243e0694, ChatGPT 822b1734, Hinge 3b8f73a0;
  Apple HIG + Google Identity branding (brand-locked button anatomy).
- 2026-08-08 · Preview-before-signup: job marketplaces (Mercor 8c9329b5,
  Remote.com f0785922, Contra d9439d22 on Mobbin) let users browse live jobs and
  soft-gate auth at the first committed action (save/apply) via modal — only
  dating apps hard-wall. Scout's trust-first positioning argues for the
  marketplace pattern. FOR PHASE 2 DECK WAVE: welcome's "Preview with sample
  data" CTA + soft auth gate on first save.

## RISKS
- (market/competitor/category threats, with source)

## REJECTED (with reasons - do not re-propose without new evidence)
- 
