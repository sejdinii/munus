...
| W3 | 3 · Studio | 3–4 | Favorites; facts-constrained tailoring + verifier; suggestions UI; tone sheet; PDF export | Generate → accept → download an honest tailored CV+letter | **NEAR-DONE (2026-08-07)** — **LIVE with real facts + Groq**: /api/facts + /api/studio/tailor (server-side key, verifier gate), AI TAILOR badge, browser-verified (Clera Senior ML Engineer → 4 grounded suggestions from real CV facts, e.g. ML_TOOLKIT/LEADERSHIP_ROLE); remaining: full accept→download pass on real kit + tone sheet on real provider |
